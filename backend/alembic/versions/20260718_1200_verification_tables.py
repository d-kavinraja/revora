...
down_revision: Union[str, None] = "pr0mpt_bu1ld3r"
...
def upgrade() -> None:
    ...
    op.create_table(
        "review_evidence",
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("verification_result_id", UUID(as_uuid=True), sa.ForeignKey("verification_results.id", ondelete="CASCADE"), index=True, nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("verification_result_id", UUID(as_uuid=True), sa.ForeignKey("verification_results.id", ondelete="CASCADE"), index=True, nullable=False, unique=True),
    ...
    op.create_table(
        ...
        sa.Column("verified_findings", sa.Integer(), server_default="0", nullable=False),
        sa.Column("rejected_findings", sa.Integer(), server_default="0", nullable=False),
        sa.Column("hallucinations_detected", sa.Integer(), server_default="0", nullable=False),
        sa.Column("false_positives_filtered", sa.Integer(), server_default="0", nullable=False),
        sa.Column("avg_confidence", sa.Float(), server_default="0.0", nullable=False),
        sa.Column("verification_duration_ms", sa.Integer(), server_default="0", nullable=False),
    )
