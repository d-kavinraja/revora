...
class VerificationResultModel(Base):
    ...
    # Relationships
    evidence: Mapped[list["ReviewEvidenceModel"]] = relationship(
        "ReviewEvidenceModel",
        back_populates="verification_result",
        cascade="all, delete-orphan",
    )
    hallucination_report: Mapped[Optional["HallucinationReportModel"]] = relationship(
        "HallucinationReportModel",
        back_populates="verification_result",
        uselist=False,
        cascade="all, delete-orphan",
    )
    false_positive_report: Mapped[Optional["FalsePositiveReportModel"]] = relationship(
...
class ReviewEvidenceModel(Base):
    ...
    # Relationships
    verification_result: Mapped["VerificationResultModel"] = relationship(
        "VerificationResultModel", back_populates="evidence"
    )
...
class HallucinationReportModel(Base):
    ...
    hallucination_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # FAKE_FILE, FAKE_API, INVALID_LINE
    details: Mapped[str] = mapped_column(Text, nullable=False)

    # Relationships
    verification_result: Mapped["VerificationResultModel"] = relationship(
        "VerificationResultModel", back_populates="hallucination_report"
    )
...
class FalsePositiveReportModel(Base):
    ...
    reason_category: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # DUPLICATE, GENERIC, LOW_CONFIDENCE
    details: Mapped[str] = mapped_column(Text, nullable=False)

    # Relationships
    verification_result: Mapped["VerificationResultModel"] = relationship(
        "VerificationResultModel", back_populates="false_positive_report"
    )
...
class VerificationMetricModel(Base):
    ...
    hallucinations_detected: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False
    )
    false_positives_filtered: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False
    )
    avg_confidence: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    verification_duration_ms: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False
    )
