...
class EvidenceSchema(BaseModel):
    evidence_type: str = Field(
        ..., description="Type of evidence e.g. SNIPPET, API_CHECK"
    )
...
class VerificationFindingSchema(BaseModel):
    ...
    severity: str = Field(
        ..., description="Severity level: HIGH, MEDIUM, LOW, CRITICAL"
    )
    issue_type: str = Field(..., description="Issue Type: SECURITY, PERFORMANCE, etc.")
    is_verified: bool = Field(
        ..., description="Whether the finding was successfully verified"
    )
    evidence: list[EvidenceSchema] = Field(
        default_factory=list, description="List of evidence strings"
    )
