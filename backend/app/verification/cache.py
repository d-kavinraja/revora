...
class VerificationCache:
    ...
    async def set_verification_result(review_id: str, result: dict[str, Any]) -> None:
        await redis_cache.set(
            f"verification:result:{review_id}", result, VERIFICATION_RESULT_TTL
        )
    ...
    async def set_confidence_scores(
        review_id: str, scores: list[dict[str, Any]]
    ) -> None:
        await redis_cache.set(
            f"verification:confidence:{review_id}", scores, CONFIDENCE_SCORE_TTL
        )
    ...
    async def set_repository_metadata(repo_url: str, metadata: dict[str, Any]) -> None:
        await redis_cache.set(
            f"verification:repo:{repo_url}", metadata, REPO_METADATA_TTL
        )
