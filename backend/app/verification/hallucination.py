...
class HallucinationDetector:
    ...
    def detect(
        ...
        if (
            re.search(
                r"\b(uses|using|import|require|depends on)\s+redis\b", description
            )
            and "redis" not in dependencies
        ):
            return {
        ...
        if re.search(r"\b(sql injection|execute sql)\b", description) and not any(
            db in dependencies
            for db in ["sqlalchemy", "psycopg2", "mysql", "pg", "sqlite", "sqlite3"]
        ):
