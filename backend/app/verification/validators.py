...
class BaseValidator:
    async def validate(
        self, finding: dict[str, Any], repo_path: str, context: dict[str, Any]
    ) -> ValidationResult:
    ...
    async def validate(
        ...
        try:
            ...
            return ValidationResult(
                is_valid=False,
                evidence="Path traversal attempt detected",
                score_modifier=-0.5,
            )
        ...
        if line_number:
            try:
                ...
                if file_size > MAX_FILE_READ_BYTES:
                    return ValidationResult(
                        is_valid=True,
                        evidence=f"File {file_path} exists, but is too large to verify line numbers (>{MAX_FILE_READ_BYTES} bytes)",
                        score_modifier=0.0,
                    )
                ...
                if 0 < line_number <= len(lines):
                    return ValidationResult(
                        is_valid=True,
                        evidence=f"Verified line {line_number} in {file_path}",
                        score_modifier=0.25,
                    )
                else:
                    return ValidationResult(
                        is_valid=False,
                        evidence=f"Line {line_number} is out of bounds for {file_path}",
                        score_modifier=-0.3,
                    )
            except Exception as e:
                return ValidationResult(
                    is_valid=False,
                    evidence=f"Could not read file {file_path}: {e!s}",
                    score_modifier=-0.2,
                )

        return ValidationResult(
            is_valid=True,
            evidence=f"Verified file {file_path} exists",
            score_modifier=0.0,
        )
...
class SecurityValidator(BaseValidator):
    ...
    async def validate(
        ...
        if finding.get("category") != "SECURITY":
            return ValidationResult(
                is_valid=True, evidence="Not a security finding", score_modifier=0.0
            )
        ...
        if not req_exists:
            return ValidationResult(
                is_valid=False,
                evidence="Target file missing for security scan",
                score_modifier=-0.5,
            )
        ...
        try:
            ...
            try:
                ...
                return ValidationResult(
                    is_valid=True,
                    evidence="Semgrep scan timed out (assuming safe or unverifiable)",
                    score_modifier=0.0,
                )
            ...
            if stdout:
                ...
                for res in results:
                    ...
                    if (
                        semgrep_line
                        and line_number
                        and abs(semgrep_line - line_number) <= 5
                    ):
                ...
                if matched_rule:
                    return ValidationResult(
                        is_valid=True,
                        evidence=f"Security issue verified by Semgrep rule: {matched_rule}",
                        score_modifier=0.4,
                    )
            ...
            return ValidationResult(
                is_valid=True,
                evidence="Security check inconclusive via static analysis",
                score_modifier=0.0,
            )
            ...
            if "sql injection" in description:
                return ValidationResult(
                    is_valid=True,
                    evidence="Security context verified (Mock fallback - Semgrep missing)",
                    score_modifier=0.1,
                )
            return ValidationResult(
                is_valid=True,
                evidence="Semgrep unavailable, skipping deep scan",
                score_modifier=0.0,
            )
        except Exception as e:
            return ValidationResult(
                is_valid=True,
                evidence=f"Security scan failed: {e!s}",
                score_modifier=0.0,
            )
...
class PerformanceValidator(BaseValidator):
    ...
    async def validate(
        ...
        if finding.get("category") != "PERFORMANCE":
            return ValidationResult(
                is_valid=True, evidence="Not a performance finding", score_modifier=0.0
            )

        return ValidationResult(
            is_valid=True,
            evidence="Performance context verified (Mock)",
            score_modifier=0.1,
        )
...
class ArchitectureValidator(BaseValidator):
    ...
    async def validate(
        ...
        if finding.get("category") != "ARCHITECTURE":
            return ValidationResult(
                is_valid=True,
                evidence="Not an architecture finding",
                score_modifier=0.0,
            )

        return ValidationResult(
            is_valid=True, evidence="Architecture context verified", score_modifier=0.1
        )
...
class RuleValidator(BaseValidator):
    ...
    async def validate(
        ...
        return ValidationResult(
            is_valid=True, evidence="Adheres to org rules", score_modifier=0.05
        )
