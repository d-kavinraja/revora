    ...
    for root, dirs, files in os.walk(repo_path):
        ...
        for f in files:
            ...
            for lang, patterns in TEST_PATTERNS.items():
                if patterns["file_pattern"].match(f) or patterns["file_pattern"].match(
                    rel_path
                ):
                    is_test = True
                    language = lang
            ...
            if language == "python":
                for m in re.finditer(TEST_PATTERNS["python"]["class_pattern"], content):
                    ...
                    graph.nodes.append(
                        GraphNode(
                            ...
                            name=m.group(1),
                            file_path=rel_path,
                            parent_id=test_file_id,
                        )
                    )
                    graph.edges.append(
                        GraphEdge(
                            source=test_file_id, target=test_class_id, type="defines"
                        )
                    )
                ...
                for m in re.finditer(TEST_PATTERNS["python"]["func_pattern"], content):
                    ...
                    graph.edges.append(
                        GraphEdge(
                            source=test_file_id, target=test_func_id, type="defines"
                        )
                    )

            elif language == "javascript":
                for m in re.finditer(
                    TEST_PATTERNS["javascript"]["it_pattern"], content
                ):
                    ...
                    graph.edges.append(
                        GraphEdge(
                            source=test_file_id, target=test_name_id, type="defines"
                        )
                    )
            ...
            if os.path.exists(source_path):
                ...
                graph.edges.append(
                    GraphEdge(source=test_file_id, target=source_file_id, type="tests")
                )
