...
def sigmoid_normalize(
    value: float, midpoint: float = 0.0, steepness: float = 1.0
) -> float:
    ...
    def normalize(self, value: float, **kwargs) -> float:
        if self._method == "min_max":
            return min_max_normalize(
                value, kwargs.get("min_val", 0.0), kwargs.get("max_val", 1.0)
            )
        elif self._method == "sigmoid":
            return sigmoid_normalize(
                value, kwargs.get("midpoint", 0.0), kwargs.get("steepness", 1.0)
            )
        elif self._method == "z_score":
            return z_score_normalize(
                value, kwargs.get("mean", 0.5), kwargs.get("std", 0.25)
            )
        return clamp(value)
