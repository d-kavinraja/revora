...
class CallGraphRetriever(BaseRetriever):
    @property
    def name(self) -> str:
        return "call_graph_retriever"
    ...
    async def retrieve(
        ...
        for changed_file in changed_files:
            ...
            callees = graph_traversal.k_hop_neighbors(
                index.call_graph,
                file_id,
                k=min(config.max_depth, 2),
            )

            for neighbor_id, depth in callees:
            ...
            for caller_id in callers:
                if caller_id in visited or not caller_id.startswith("file:"):
                    continue
                if caller_id == file_id:
                    continue
                visited.add(caller_id)

                caller_file = caller_id.replace("file:", "", 1)
                if caller_file in changed_files:
                    continue
