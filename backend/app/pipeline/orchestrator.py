...
class ReviewPipeline:
    ...
    async def execute(
        ...
        try:
            ...
            await emitter.emit(
                "validating_ai_configuration", "running", EventType.STAGE_START
            )
            ...
            await emitter.emit_log(
                "preparing_review", f"Starting review for PR #{pr_number}"
            )
            ...
            try:
                ...
                if check_run_id:
                    async with AsyncSessionLocal() as db:
                        ...
                        await db.commit()
                        logger.info(
                            f"Saved check_run_id={check_run_id} to review {review_id}"
                        )
            except Exception as e:
            ...
            index = await self._stage_indexing(
                emitter, repo_path, owner, repo_name, head_sha
            )
            ...
            if any(p in model_lower for p in _WEAK_MODEL_PATTERNS):
                ...
                logger.warning(
                    f"Weak model detected for review {review_id}: {weak_msg}"
                )
                await emitter.emit(
            ...
            await _record_timeline(
                "pipeline_complete",
                "completed",
                message=f"Total duration: {duration_ms:.0f}ms",
                duration_ms=duration_ms,
            )
    ...
    async def _stage_clone(self, emitter, clone_url, token, head_sha=None):
        ...
        try:
            ...
            await emitter.emit(
                "cloning_repository", "completed", EventType.STAGE_COMPLETE
            )
            return repo_path
        except Exception as e:
            logger.warning(f"Clone failed: {e}")
            await emitter.emit(
                "cloning_repository", "failed", EventType.STAGE_FAILED, message=str(e)
            )
            return None
    ...
    async def _stage_intelligence(self, emitter, repo_path):
        ...
        if not repo_path:
            await emitter.emit(
                "analyzing_repository", "skipped", EventType.STAGE_SKIPPED
            )
            return {}
        ...
        try:
            ...
            await emitter.emit(
                ...
                "completed",
                EventType.STAGE_COMPLETE,
                metrics=intelligence_data,
            )
            return intelligence_data
        except Exception as e:
            logger.warning(f"Intelligence analysis failed: {e}")
            await emitter.emit(
                "analyzing_repository", "failed", EventType.STAGE_FAILED, message=str(e)
            )
            return {}
    ...
    async def _stage_indexing(self, emitter, repo_path, owner, repo_name, head_sha):
        ...
        if not repo_path:
            await emitter.emit(
                "indexing_repository", "skipped", EventType.STAGE_SKIPPED
            )
            return None
        ...
        try:
            ...
            await emitter.emit(
                "indexing_repository", "failed", EventType.STAGE_FAILED, message=str(e)
            )
            return None
    ...
    async def _stage_knowledge(self, emitter, owner, repo_name):
        ...
        await emitter.emit(
            "retrieving_repository_knowledge", "running", EventType.STAGE_START
        )
        ...
        try:
            ...
            await emitter.emit(
                "retrieving_repository_knowledge",
                "failed",
    ...
    async def _stage_retrieval(
        self, emitter, index, diff_content, repo_id=None, budget=16000
    ):
        ...
        if not index:
            await emitter.emit(
                "finding_related_files", "skipped", EventType.STAGE_SKIPPED
            )
            return None
        ...
        try:
            ...
            changed_files = list(
                set(re.findall(r"diff --git a/(.*?) b/", diff_content))
            )
    ...
    async def _stage_prompt(
        ...
        try:
            ...
            await emitter.emit(
                "building_prompt", "completed", metrics={"tokens": prompt.total_tokens}
            )
            return prompt
        except Exception as e:
            logger.warning(f"Prompt building failed: {e}")
            await emitter.emit(
                "building_prompt", "failed", EventType.STAGE_FAILED, message=str(e)
            )
    ...
    async def _stage_verification(
        self, emitter, ai_response, repo_path, diff_content, review_id=None
    ):
        """Stage: Verification."""
        await emitter.emit(
            "running_verification_agent", "running", EventType.STAGE_START
        )
        ...
        try:
            ...
            verified = await verification_engine.verify(
                ai_response, repo_path, changed_files, context={"review_id": review_id}
            )
            ...
            await emitter.emit(
                "running_verification_agent", "completed", metrics=verified.to_dict()
            )
            return verified
        except Exception as e:
            logger.warning(f"Verification failed: {e}")
            await emitter.emit(
                "running_verification_agent",
                "failed",
                EventType.STAGE_FAILED,
                message=str(e),
            )
    ...
    async def _publish_or_update_pr_review(
        ...
        if previous_review_id:
            try:
                ...
                logger.info(
                    f"Updated existing PR review {previous_review_id} on PR #{pr_number}"
                )
    ...
    async def _stage_publish(
        ...
        """
        await emitter.emit(
            "generating_review_summary", "running", EventType.STAGE_START
        )
        ...
        github_comments = [
            ...
            for c in review_summary.comments
            if c.line
        ]
        ...
        try:
            ...
            try:
                ...
                if github_comments:
                    logger.info(
                        "Attempting to publish review without inline comments as fallback"
                    )
                    try:
                        await self._publish_or_update_pr_review(
                            installation_id=installation_id,
            ...
            await emitter.emit(
                "publishing_review", "failed", EventType.STAGE_FAILED, message=str(e)
            )
        ...
        try:
            if prev_comment_id:
                try:
                    ...
                    comment_id = prev_comment_id
                    logger.info(
                        f"Updated existing Revora comment {prev_comment_id} on PR #{pr_number}"
                    )
                except Exception as e:
            ...
            logger.error(
                f"Failed to upsert Revora summary comment on PR #{pr_number}: {e}"
            )
    ...
    async def _save_completed(
                ...
                if db_review:
                    ...
                    try:
                        ...
                        if db_review.started_at and db_review.completed_at:
                            duration_ms = int(
                                (
                                    db_review.completed_at - db_review.started_at
                                ).total_seconds()
                                * 1000
                            )
        ...
        if settings.USAGE_ANALYTICS_ENABLED:
            try:
                ...
                logger.info(
                    f"[usage] Logging for review {review_id}: provider={llm_response.provider} "
                    f"input={input_tok} output={output_tok} user_id={user_id}"
                )

                input_cost = round(
                    cost_estimator.estimate(llm_response.provider, input_tok, 0), 8
                )
                output_cost = round(
                    cost_estimator.estimate(llm_response.provider, 0, output_tok), 8
                )
                ...
                if parsed_user_id:
                    async with AsyncSessionLocal() as usage_db:
                        ...
                        logger.info(
                            f"[usage] token_manager.record_usage committed for review {review_id}"
                        )
                    ...
                    async with AsyncSessionLocal() as log_db:
                        ...
                        logger.info(
                            f"[usage] usage_tracker.log_request committed for review {review_id}"
                        )
                else:
                    logger.warning(
                        f"[usage] No valid user_id for review {review_id}, skipping usage log"
                    )
            except Exception as ue:
                logger.error(
                    f"[usage] Failed to record usage stats: {ue}", exc_info=True
                )
        else:
            logger.debug(
                "[usage] USAGE_ANALYTICS_ENABLED=False, skipping usage recording"
            )
            ...
            async with AsyncSessionLocal() as db:
                ...
                if db_review:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to mark execution failed for review {review_id}: {e}"
                        )
        except Exception as e:
