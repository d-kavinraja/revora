...
class UsageTracker:
    ...
    def _apply_filters(
        ...
        if api_key_id:
            try:
                parsed_api_key_id = (
                    uuid.UUID(api_key_id) if isinstance(api_key_id, str) else api_key_id
                )
    ...
    async def get_user_requests(
        ...
        query = self._apply_filters(
            query, provider, model, api_key_id, repo_id, start_date, end_date
        )
        query = (
            query.order_by(LLMRequestLog.started_at.desc()).offset(offset).limit(limit)
        )
    ...
    async def get_error_summary(
        ...
        by_provider = {}
        for e in errors:
            by_type[e.error_type or "unknown"] = (
                by_type.get(e.error_type or "unknown", 0) + 1
            )
            by_provider[e.provider] = by_provider.get(e.provider, 0) + 1

        total_query = select(func.count(LLMRequestLog.id)).where(
            LLMRequestLog.user_id == user_id
        )
        total_query = self._apply_filters(
            total_query, provider, model, api_key_id, repo_id, start_date, end_date
        )
            ...
            result_list.append(
                {
                    ...
                    "avg_latency_ms": (
                        round(data["latency"] / data["count"], 2)
                        if data["count"]
                        else 0
                    ),
                }
