...
@pytest.mark.asyncio
async def test_anthropic_completion_success(db_session, mock_llm_completion):
    # Tier 1 - Feature Coverage: Anthropic completion
    prompt = CompiledPrompt(system_prompt="System", user_prompt="Hello", total_tokens=15)
    response = await llm_orchestrator.complete(prompt, str(MOCK_USER_ID), preferred_provider="anthropic")
...
@pytest.mark.asyncio
async def test_gemini_completion_success(db_session, mock_llm_completion):
    # Tier 1 - Feature Coverage: Gemini completion
    prompt = CompiledPrompt(system_prompt="System", user_prompt="Hello", total_tokens=20)
    response = await llm_orchestrator.complete(prompt, str(MOCK_USER_ID), preferred_provider="gemini")
...
@pytest.mark.asyncio
async def test_groq_completion_success(db_session, mock_llm_completion):
    # Tier 1 - Feature Coverage: Groq completion
    prompt = CompiledPrompt(system_prompt="System", user_prompt="Hello", total_tokens=25)
    response = await llm_orchestrator.complete(prompt, str(MOCK_USER_ID), preferred_provider="groq")
...
@pytest.mark.asyncio
async def test_deepseek_completion_success(db_session, mock_llm_completion):
    # Tier 1 - Feature Coverage: DeepSeek completion
    prompt = CompiledPrompt(system_prompt="System", user_prompt="Hello", total_tokens=30)
    response = await llm_orchestrator.complete(prompt, str(MOCK_USER_ID), preferred_provider="deepseek")
...
@pytest.mark.asyncio
async def test_usage_stats_persistence_on_success(db_session, mock_llm_completion):
    ...
    result = await db_session.execute(select(LlmUsage).where(LlmUsage.user_id == MOCK_USER_ID))
...
@pytest.mark.asyncio
async def test_retry_logic_on_rate_limit(db_session, monkeypatch, mock_llm_completion):
    ...
    monkeypatch.setattr("app.ai.llm.api_key_service.get_decrypted_key", lambda *args: "fail_once_key")
...
@pytest.mark.asyncio
async def test_fallback_to_next_priority_provider(db_session, monkeypatch, mock_llm_completion):
    # Tier 2 - Boundary/Corner: Provider fallback when preferred fails persistently
    prompt = CompiledPrompt(system_prompt="System", user_prompt="Hello", total_tokens=10)
    
    # Force preferred provider to fail persistently
    monkeypatch.setattr("app.ai.llm.api_key_service.get_decrypted_key", lambda *args: "fail_always_key")
...
@pytest.mark.asyncio
async def test_all_providers_fail_raises_runtime_error(db_session, monkeypatch, mock_llm_completion):
    ...
    with pytest.raises(RuntimeError) as exc_info:
        await llm_orchestrator.complete(prompt, str(MOCK_USER_ID))
    
    assert "All LLM providers failed" in str(exc_info.value)

@pytest.mark.asyncio
async def test_cost_estimation_calculation(db_session, mock_llm_completion):
    ...
    # Estimated cost for openai input: 0.0025 per 1K, output: 0.01 per 1K
    # output tokens = len(OpenAI response) // 4 = 29 // 4 = 7
    # input cost = 1.0 * 0.0025 = 0.0025
    # output cost = 0.007 * 0.01 = 0.00007
    # Total cost = 0.00257
    response = await llm_orchestrator.complete(prompt, str(MOCK_USER_ID), preferred_provider="openai")
    assert abs(response.estimated_cost_usd - 0.00257) < 0.0001

@pytest.mark.asyncio
async def test_orchestrator_updates_success_rate_on_failure(db_session, monkeypatch, mock_llm_completion):
    ...
    try:
        await llm_orchestrator.complete(prompt, str(MOCK_USER_ID), preferred_provider="groq")
    except Exception:
        pass
...
@pytest.mark.asyncio
async def test_orchestrator_updates_success_rate_on_success(db_session, mock_llm_completion):
    ...
    await llm_orchestrator.complete(prompt, str(MOCK_USER_ID), preferred_provider="openai")
    
    assert provider_config.success_rate == 0.95
