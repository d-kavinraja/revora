...
function StatCard({ label, value, icon, gradient }: { label: string; value: number | string; icon: React.ReactNode; gradient?: string }) {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/5 bg-zinc-950 p-6 shadow-xl">
...
export default function Dashboard() {
  ...
  useEffect(() => {
    fetchData();
    // Poll every 10 seconds for live review status updates
            ...
            {reviews.map((review) => (
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <svg className="w-4 h-4 text-zinc-600 group-hover:text-indigo-400 transition-colors shrink-0 mt-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                  </svg>
                </div>
              </Link>
